function ax = plotCDProj(allrez,rez,sav,spacename,plotmiss,plotno,rez_2afc,obj)
...
%         temperror(:,j) = getCI(temp_);
      ...
      shadedErrorBar(obj(1).time,tempmean(:,1),temperror(:,1),{'Color',clrs.rhit,'LineWidth',lw},alph, ax(i))
      shadedErrorBar(obj(1).time,tempmean(:,2),temperror(:,2),{'Color',clrs.lhit,'LineWidth',lw},alph, ax(i))
    if plotmiss
        sm = 51;
        smtype = 'zeropad';
        shadedErrorBar(obj(1).time,mySmooth(tempmean(:,3),sm,smtype),mySmooth(temperror(:,3),sm,smtype)/2,{'Color',clrs.rhit,'LineWidth',lw,'LineStyle',':'},alph, ax(i));
        shadedErrorBar(obj(1).time,mySmooth(tempmean(:,4),sm,smtype),mySmooth(temperror(:,4),sm,smtype)/2,{'Color',clrs.lhit,'LineWidth',lw,'LineStyle',':'},alph, ax(i))
    end
