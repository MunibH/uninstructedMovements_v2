function acc = choiceDecoder_v2(obj,rez,params,cv,cond2use)
...
for sessix = 1:numel(rez)
    ...
    % -- null space
    % input
    X = rez(sessix).N_null(:,trials.all,:);
    ...
    disp('potent')
    % input
    X = rez(sessix).N_potent(:,trials.all,:);
...
for i = 1:numel(fns)
    mu = mean(acc.(fns{i}),2);
    sigma = std(acc.(fns{i}),[],2) ./ sqrt(numel(rez));
...
title('Choice decoding from NP','FontSize',8)
