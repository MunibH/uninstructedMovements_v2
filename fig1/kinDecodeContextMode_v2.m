...
% data sets
par.train = 0.7; % fraction of trials (just using cross-val here, matlab's kFoldPredict uses held out data for testing)
...
for sessix = 1:numel(meta) % 7
    ...
    par.trials.dr = params(sessix).trialid{par.cond2use(1)}; % CHANGE %%%%%%%%%%%%%%%%%
    par.trials.wc = params(sessix).trialid{par.cond2use(2)}; % CHANGE %%%%%%%%%%%%%%%%%
    ...
    dr_trials = ismember(par.trials.test,par.trials.dr);
    wc_trials = ismember(par.trials.test,par.trials.wc);

    ydr = y(:,dr_trials) - Y.mu; % only need to add back mean for cd_context, this is just for visualization - make it match figure2
    ywc = y(:,wc_trials);

    yhatdr = yhat(:,dr_trials) - Y.mu; % same as above
    yhatwc = yhat(:,wc_trials);
