...
%% trials to use - only want to use half DR trials
% save other half DR and all WC for testing

rez.trials.all = 1:obj.bp.Ntrials;

rez.trials.dr = cell2mat(params.trialid(cond2use)');
rez.trials.dr1 = randsample(rez.trials.dr,round(numel(rez.trials.dr)/2),false);

rez.trials.dr2 = rez.trials.dr(~ismember(rez.trials.dr,rez.trials.dr1));

trials = rez.trials.dr1; % trials to use to estimate n/p
