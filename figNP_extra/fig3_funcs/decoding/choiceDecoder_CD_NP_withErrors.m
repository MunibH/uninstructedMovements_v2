function acc = choiceDecoder_CD_NP_withErrors(obj,rez,null,potent,params,cv,cond2use,cdix)
...
for sessix = 1:numel(rez)
    ...
    if nmiss == 0 % min number of trials in a condition is zero, so skip that session
        continue
    end
    ...
    Y = [ones(nhits,1) ; -ones(nhits,1) ; -ones(nmiss,1) ; ones(nmiss,1)]; % right hits, left hits, right miss, left miss
