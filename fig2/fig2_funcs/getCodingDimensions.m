function rez = getCodingDimensions(obj,params,cond2use,cond2proj,proj_trialdat)
% 
...
for sessix = 1:numel(obj)
    ...
    % single trial data
    dims = size(obj(sessix).trialdat);
    trialdat = reshape(permute(obj(sessix).trialdat,[1 3 2]),dims(1)*dims(3),dims(2)); % (time*trials,clu)
    temp = trialdat * rez(sessix).cd_mode_orth;
    rez(sessix).cd_proj_trialdat = reshape(temp, dims(1), dims(3), numel(cd_labels));
