...
for ifeat = 1:numel(featGroups)
    ...
    for sessix = 1:numel(obj)
        ...
        trix.rmiss = params(sessix).trialid{5};
        trix.lmiss = params(sessix).trialid{3};
        trix.left = params(sessix).trialid{4};

        % train on right trials, test on rmiss and lmiss
        nhits = numel(trix.right);
        in.train.y = [ones(nhits,1)];
        in.train.X = kin(sessix).dat(:,trix.right,rez.featix);

        nrmiss = numel(trix.rmiss);
        nlmiss = numel(trix.lmiss);
        in.test.y = [-ones(nrmiss,1) ; ones(nlmiss,1)];
        in.test.X = kin(sessix).dat(:,[trix.rmiss ; trix.lmiss],rez.featix);

        % decoding

        acc(:,sessix,ifeat) = DLC_ChoiceDecoder_testing(in,rez);


        % train on left trials, test on rmiss and lmiss
